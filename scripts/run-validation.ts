...
async function main() {
  ...
  // Fetch shared intelligence via direct lookup (bypasses MCP HTTP loop)
  console.log(`\n🧠 Fetching verified intelligence via direct lookup...`);
  const { getAgentIntelligenceContext } = await import('../lib/intelligence/agent-context');
  const intelContext = await getAgentIntelligenceContext(enrichedProject as any).catch(() => ({ promptBlock: '', tablesUsed: [] as string[], raw: {} as any, citations: '' }));
  (enrichedProject as any)._mcpIntelBlock = intelContext.promptBlock;
  console.log(`  Intel: ${intelContext.tablesUsed.length > 0 ? '✅' : '⚠️'} ${intelContext.tablesUsed.length} tables, ${intelContext.promptBlock.length} chars\n`);
