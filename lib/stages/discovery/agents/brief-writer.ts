...
We recommend the following for immediate Feasibility Study:

${topPrograms.map((p, i) => `   ${i + 1}. **${p.programTitle}** (${p.scores.composite}/10 — ${p.tier.replace(/_/g, ' ')})`).join('\n')}

**3. Engage Wavelength Feasibility Study**  
Feasibility Study will confirm employer demand, model enrollment and revenue, conduct detailed feasibility analysis, and deliver a go/no-go recommendation with full supporting data. This is the critical step before committing development resources.
